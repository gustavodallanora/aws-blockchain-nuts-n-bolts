...
# Fetch secret
aws secretsmanager get-secret-value --secret-id ${HTR_WALLETS_SECRET_NAME} --query SecretString --output text > seeds
if [ $? -eq 0 ]
then
  echo "Found ${HTR_WALLETS_SECRET_NAME} secret, continuing..."
else
  echo ""
  echo "Aborting wallet start..."
  echo "You must configure ${HTR_WALLETS_SECRET_NAME} secret using this instance, delete the stack and create it again."
  exit 1;
fi
...
sed -i -E "s/(network: ').*(',)/\1${HTR_NETWORK_NAME}\2/" src/config.js && cat src/config.js | grep network:
sed -i -E "s,(server: ').*('\,),\1${HTR_NETWORK_SERVER}\2," src/config.js && cat src/config.js | grep server:
